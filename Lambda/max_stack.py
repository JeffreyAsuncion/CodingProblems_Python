"""
You've encountered a situation 
where you want to easily be able to pull the
largest integer from a stack.

You already have a `Stack` class 
that you've implemented using a dynamic array.
Use this `Stack` class to implement a new class 
`MaxStack` with a method
`get_max()` that 
returns the largest element in the stack. 

`get_max()` should not remove the item.

*Note: Your stacks will contain only integers. 
...
class MaxStack:
    def __init__(self):
        # Your code here
        # keep track of a normal stack to the standard operations
        self.stack = Stack()
    ...
    def get_max(self):
        """The last item in max_stack is the max item in our stack."""
        # Your code here
        # peek the top of the max stack
